...
class DWA:
    ...
    def dwa_control(self, state, object, lanes):
        """
        Dynamic Window Approach control
        """
        # self._front_image = image
        # self._depth_image = dep_image
        self._obj_dis = object
        self._lanes = lanes
        # image proccesing 
    ...
    def _calc_control_and_trajectory(self, dw, state):
        ...
        print("Lane Cost : {}".format(self._calc_lane_cost(best_trajectory, x_init)))
        print("Goal Cost : {}".format(self._calc_to_goal_cost(best_trajectory)))
    ...
    def _calc_lane_cost(self, trajectory, x_init):
        ...
        for lane in self._lanes:
            rm = []
            for t in trajectory:
                rm.append(abs(self._perpendicular_distance_from_line(t, [[lane.x1, lane.y1], [lane.x2, lane.y2]])))
            r.append(rm)
        r = np.array(r)
